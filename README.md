<!-- <head><link rel="stylesheet" href="styles.css"></head> -->
EasyTrace (This is an android application which is related long distance bus tracking and booking system).
<p>Youtube link: https://youtu.be/wwgFBmV1_Sg</p>

Click the image below for the demo video

<!-- [![This is a Youtube Link](https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/youtube%20thumbnail.png){: width="200" height="150"}](https://youtu.be/wwgFBmV1_Sg) -->

<div align="center">
<a href="https://youtu.be/wwgFBmV1_Sg"><img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/youtube%20thumbnail.png" width="475" height="325"></a>
</div>
<p align="center">This is an Android application for passengers who
travel by bus and for bus operators. the passengers
can track the bus in real-time and be seat reservations
also. This is a solution for Long distance bus system in Sri Lanka. This is a second year group project. 
I was doing a software developer role.
   
Developed by using <b>Java, XML, Firebase, Google Maps
   API</b></p>


-Functional requirements

    •	The user shall be able to download the app to the mobile phone. First the user has to register/create an account.  
    •	The system should store the user’s details.  
    •	The user shall be able to update / delete the user account.  
    •	The system must be able to shoe information to user in real time.  
    •	The system must be able to process the position data received from bus positioning module and calculate the estimated time to   the user.  
    •	The system must be able to allow user to retrieve information through mobile devices.  
    •	The system must be able to do mapping with the position data retrieved from bus positioning module and show the bus position in a map view to the user.  
    •	The passenger should be able to make the payments through the wallet of the system.  
    •	The system should keep track of the transactions done by the user.  
    •	The wallet shall allow the user to make remote payments.  
    •	User should be able to see location of the buses using GPS.   
    •	The passenger should be able to check available seats before entering into the bus through the app.  
    •	Then passenger must scan the NFC code on the door to enter the bus and direct the passenger to the system through the NFC.  
    •	The system should be real time in order to deduct or increase the balance when the passenger do a transaction.  

-Non-Functional requirements

    •	Data integrity and accuracy- The system should provide the accurate estimated bus arrival time to the user.  
    •	Performance -The system should be able to increase the efficiency and performance of bus services.   
    •	Serviceability -The system should allow user to access information in anywhere in anytime.  
    •	Availability -The system should be 24/7 available without any failure.   
    •	Usability- The system should be easy to use for all age types.   
    •	Capacity and scalability – the system should allow users to login and do the payments at the same time without any system failure.  
    •	Security- The system should be highly secured as it contains sensitive details of the user such as credit / debit card, NIC, license card details and encrypted with a password to avoid unauthorized access and secure passenger’s details.  
    •	Manageability -Easily updatable  
    •	The system should reduce the paperwork done by bus management system.  
    •	The software should be portable  

 
<p align="center" style="font-size: 40px;">UI designs</p>

#Login Screen

<div class="image-container">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/3.jpg" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/4.jpg" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/5.png" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/6.png" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/8.jpg" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/7.png" width="500" height="300">
</div>


#Functions

<div class="image-container">
 <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/1%20(1).png" width="240" height="500">
 <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/11.jpg" width="240" height="500">
   <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/14.jpg" width="240" height="500">
    <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/15.png" width="240" height="500">
    <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/16.png" width="240" height="500">
    <img src="https://raw.githubusercontent.com/Nivesh98/EasyTrace/main/Assests/17.png" width="240" height="500">
<div>
  


